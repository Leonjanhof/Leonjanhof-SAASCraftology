...
import { getVotingProfiles } from "@/lib/api/profiles";
...
const VotingDashboard = () => {
  ...
  const profileType = location.state?.profileType || "voting";

  useEffect(() => {
    const loadProfiles = async () => {
      setIsLoading(true);
      try {
        const { data: votingProfiles, error } = await getVotingProfiles();
          ...
          <div className="flex items-center space-x-2">
            ...
            >
              {currentProfile ? (
                <>
                  Your{" "}
                  <span className="text-green-400">{currentProfile.name}</span>{" "}
                  <span className="text-blue-500">Voting</span> Dashboard
                </>
              ) : (
                "Voting Dashboard"
              )}
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                {profiles.map((profile) => (
                  ...
                  >
                    {profile.name}
                    <span className="ml-2 text-xs text-blue-500 px-1.5 py-0.5 rounded-full bg-blue-50">
                      Voting
                    </span>
                  </DropdownMenuItem>
                ))}
