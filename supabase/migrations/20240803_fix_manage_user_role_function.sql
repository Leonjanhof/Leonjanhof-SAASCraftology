...
DROP FUNCTION IF EXISTS manage_user_role(UUID, TEXT, TEXT);

-- Create or replace the manage_user_role function with fixed column references
...
BEGIN
  ...
  SELECT role_name INTO admin_role FROM user_roles WHERE user_roles.user_id = admin_user_id;
